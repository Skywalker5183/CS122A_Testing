#include "../include/Texting_app.h"

static void back_pressed(void);
static void make_Texting_app();
...
void make_Texting_button(void)
{
    ...
    lv_img_set_src(img, &Text_logo);          // Image appears on top of button

    lv_obj_center(img);                          // Centers image

    lv_obj_set_pos(btn, 200, 10);
...
static void make_Texting_app()
{
    ...
    lv_label_set_text(receive_label, "Received");
    lv_obj_set_style_text_color(receive_label, lv_color_hex(0x000000), 0);
    lv_obj_set_style_bg_color(receive_label, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_bg_opa(receive_label, LV_OPA_COVER, 0);
    lv_obj_align(receive_label, LV_ALIGN_TOP_LEFT, 0, 30);

    // Text area for received messages (read-only)
    receive_ta = lv_textarea_create(parent);
    lv_textarea_set_text(receive_ta, "No Message");
    lv_obj_set_style_radius(receive_ta, 0, 0);
    lv_obj_set_style_border_width(receive_ta, 5, LV_PART_MAIN);
    lv_obj_set_pos(receive_ta, 0, 45);
    lv_obj_set_size(receive_ta, LV_PCT(100), (LV_VER_RES - 30)/3);
    /* Prevent user editing and cursor by disabling the textarea */
    lv_obj_add_state(receive_ta, LV_STATE_DISABLED);

        // -------------------- Text Send ----------------------- //
    text_area = lv_textarea_create(parent);

    // Label for sending messages
    send_label = lv_label_create(parent);
    lv_label_set_text(send_label, "Send");
    lv_obj_set_style_text_color(send_label, lv_color_hex(0x000000), 0);
    lv_obj_set_style_bg_color(send_label, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_bg_opa(send_label, LV_OPA_COVER, 0);
    lv_obj_align(send_label, LV_ALIGN_TOP_LEFT, 0, LV_VER_RES - (LV_VER_RES - 30)/3  - (LV_VER_RES - 30)/3 + 15);

    lv_obj_set_style_radius(text_area, 0, 0);
    lv_obj_set_style_border_width(text_area, 5, LV_PART_MAIN);

    // Leave room for back button which has a size of 30x30
    lv_obj_set_pos(text_area, 0, LV_VER_RES - (LV_VER_RES - 30)/3  - (LV_VER_RES - 30)/3 + 30);

    // LV_PCT(100) means make the width 100% of the parent object's width.
    lv_obj_set_size(text_area, LV_PCT(100)/3, (LV_VER_RES - 30)/3);

    lv_textarea_set_placeholder_text(text_area, "Text here");
    ...
    lv_obj_add_event_cb(text_area, ta_event_cb, LV_EVENT_ALL, kb);
     ...
     /* Ensure the send textarea is focused on open so the cursor starts here
         and show the keyboard immediately. */
     lv_keyboard_set_textarea(kb, text_area);
     lv_obj_add_state(text_area, LV_STATE_FOCUSED);
     lv_obj_clear_state(receive_ta, LV_STATE_FOCUSED);
     lv_obj_clear_flag(kb, LV_OBJ_FLAG_HIDDEN);
    ...
    lv_label_set_text(title_label, "Texting");
